...
describe('ProviderController', () => {
  ...
  it('invokes GET /api/v1/providers', async () => {
    const res = await client.get('/api/v1/providers').expect(200);
  ...
  it('invokes GET /api/v1/providers/{:id}', async () => {
    const res = await client.get('/api/v1/providers/1').expect(200);
  ...
  it('invokes POST /api/v1/providers', async () => {
    const initRes = await client.get('/api/v1/providers').expect(200);
    ...
    const res1 = await client.post('/api/v1/providers').send(providerData).expect(200);

    const provider = res1.body as ProviderDto;
    expect(provider || null).to.not.be.null();
    expect(provider.id || null).to.not.be.null();

    const res2 = await client.get('/api/v1/providers').expect(200);

    const providers = res2.body as ProviderDto[];
    expect(providers.length).to.equal(initProviders.length + 1);

    const res3 = await client.get(`/api/v1/providers/${provider.id}`).expect(200);
  ...
  it('invokes PUT /api/v1/providers/{:id}', async () => {
    ...
    const res1 = await client.put(`/api/v1/providers/${provider.id}`).send({
      id: provider.id,
      name: newName,
    ...
    const res2 = await client.get(`/api/v1/providers/${provider.id}`).expect(200);
  ...
  it('invokes DEL /api/v1/providers/{:id}', async () => {
    const initRes = await client.get('/api/v1/providers').expect(200);
    const initProviders = initRes.body as ProviderDto[];

    const res = await client.delete('/api/v1/providers/3').expect(200);
    const ok = res.body;
    expect(ok || null).to.not.be.null();
    expect(ok).to.equal(true);

    const finalRes = await client.get('/api/v1/providers').expect(200);
    const finalProviders = finalRes.body as ProviderDto[];
    expect(finalProviders.length).to.equal(initProviders.length - 1);
  });

  it('invokes GET /api/v1/providers/{:id}/images', async () => {
    const res = await client.get('/api/v1/providers/1/images').expect(200);
  ...
  it('invokes GET /api/v1/providers/{:id}/images/{:imageId}/plans', async () => {
    const res = await client.get('/api/v1/providers/1/images/1/plans').expect(200);
  ...
  it('invokes GET /api/v1/providers/{:id}/flavours', async () => {
    const res = await client.get('/api/v1/providers/1/flavours').expect(200);
